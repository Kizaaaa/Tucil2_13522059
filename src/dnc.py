...
def DNCmain(points,iterate) :
    ...
    for i in range(len(points)-1) :
        plt.plot([points[i][0], points[i+1][0]], [points[i][1], points[i+1][1]], marker='o', linestyle='-', color="black")
    for i in range(len(pointAkhir)-1) :
        plt.plot([pointAkhir[i][0], pointAkhir[i+1][0]], [pointAkhir[i][1], pointAkhir[i+1][1]], marker='o', linestyle='-', color="black")

    plt.text(0,0,f"Waktu yang diperlukan: {totalWaktu:.4f} detik")
